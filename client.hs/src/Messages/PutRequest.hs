...
module Messages.PutRequest (PutRequest(..)) where
import Prelude ((+), (/))
...
data PutRequest = PutRequest{key :: !(P'.Utf8), value :: !(P'.Utf8)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable PutRequest where
  mergeAppend (PutRequest x'1 x'2) (PutRequest y'1 y'2) = PutRequest (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
 
instance P'.Default PutRequest where
  defaultValue = PutRequest P'.defaultValue P'.defaultValue
 
instance P'.Wire PutRequest where
  wireSize ft' self'@(PutRequest x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(PutRequest x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
